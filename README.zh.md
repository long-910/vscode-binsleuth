...
适用于使用 Open VSX Registry 的编辑器（VSCodium、Gitpod、Eclipse Theia 等）：

1. 在编辑器中打开扩展视图。
2. 搜索 **BinSleuth**。
3. 点击**安装**。

或直接从 [Open VSX 页面](https://open-vsx.org/extension/long-910/vscode-binsleuth) 安装。
...
| 发布到 Open VSX Registry | ✅ v0.2.0 |
| CI 修复：Open VSX 发布工作流 | ✅ v0.2.1 |
| 可配置桥接二进制路径 | 🔲 计划中 |
| PE / Mach-O 格式徽章 | 🔲 计划中 |
| 差异视图（比较两个二进制文件） | 🔲 计划中 |

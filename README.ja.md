...
**1. Rust ブリッジをビルド**（初回のみ・Rust 1.85+ が必要）

```bash
git clone https://github.com/long-910/vscode-binsleuth.git
cd vscode-binsleuth
npm install
npm run build:rust      # src-rust/ → bin/binsleuth-bridge をコンパイル
npm run compile         # TypeScript をコンパイル
```

**2. Extension Development Host を開く**

VS Code で **F5** を押します。
拡張機能が有効になった新しいウィンドウが開きます。

**3. バイナリを解析する**

`.elf`、`.exe`、`.dll`、`.so`、`.bin`、`.o`、`.a`、`.dylib`、`.out` のいずれかのファイルを開くと、BinSleuth サイドバーが自動的に更新されます。

> **ヒント:** アクティビティバーの ⊕ アイコンをクリックすると、いつでも BinSleuth パネルを開けます。
...
### VS Code を WSL 内で実行する場合（Remote - WSL）

そのまま動作します。
ブリッジバイナリは Linux ELF（`bin/binsleuth-bridge`）として直接呼び出されます。

### VS Code を Windows ネイティブで実行する場合

| シナリオ | 使用バイナリ | WSL が必要か |
|---------|------------|:-----------:|
| `win32-x64` VSIX からインストール | `binsleuth-bridge.exe`（ネイティブ） | 不要 |
| WSL でソースをビルドし Windows VS Code でテスト | `binsleuth-bridge`（Linux ELF、`wsl.exe` 経由） | 必要 |
...
cd vscode-binsleuth
npm install
npm run build:rust      # Rust ブリッジ → bin/binsleuth-bridge（Linux/macOS）
                        #                → bin/binsleuth-bridge.exe（Windows）
npm run compile         # TypeScript → out/
```

**F5** で Extension Development Host を起動します。

### VSIX をビルドして任意の VS Code にインストール
...
> **Windows の場合:** VSIX を Windows ドライブへコピーしてからインストールしてください。
> ```bash
> npm run package:vsix-win   # ビルド + Windows の Downloads フォルダへ自動コピー
> ```

`.vsix` のインストール:
**拡張機能（Ctrl+Shift+X）** → **⋯** → **VSIX からインストール…**
...
```bash
# TypeScript のウォッチ（保存時に自動再コンパイル）
npm run watch

# src-rust/ を編集した後、Rust ブリッジを再ビルド
npm run build:rust

# Extension Development Host を起動
# → VS Code で F5 を押す（設定: .vscode/launch.json）
```
...
│   └── src/main.rs         # CLI: バイナリを読み込み → JSON を標準出力
├── src/
│   ├── extension.ts        # activate()、コマンド、自動検出、パス正規化
│   └── panel.ts            # WebviewViewProvider + Webview HTML/CSS/JS
├── bin/                    # コンパイル済みブリッジバイナリ（git 管理外）
├── l10n/                   # 翻訳バンドル（日本語・中国語）
├── resources/
│   └── icon.svg            # アクティビティバーアイコン
└── .vscode/
    ├── launch.json         # F5 デバッグ設定
    └── tasks.json          # TypeScript ビルドタスク
```

Rust ブリッジは JSON オブジェクト 1 個を標準出力に書き出して終了します。
拡張機能は `child_process.execFile` でこれを読み込み、Webview に渡します。
...
| WSL / Windows ネイティブ VS Code サポート | ✅ v0.1.0 |
| 多言語対応（日本語・中国語） | ✅ v0.1.0 |
| VS Code Marketplace への公開 | 🔲 予定 |
| ブリッジバイナリパスの設定 | 🔲 予定 |
| PE / Mach-O 形式バッジ | 🔲 予定 |
| 差分ビュー（2つのバイナリの比較） | 🔲 予定 |
